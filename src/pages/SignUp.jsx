...
export function SignUp(){
  ...
  const navigate=useNavigate();
    ...
    function handleSignUp(e){
        e.preventDefault();
        // console.log("helo");
        const validationError=validateSignUp2(firstName,lastName,email,password)
        setErrors(validationError);

    }
   
    function validateSignUp2(firstName,lastName,email,password){
      let error={};
    ...
    return (
        ...
        style={{ width: "90%", maxWidth: "20rem" }} noValidate
      >
        ...
        <div class="flex-column margin-bottom">
          ...
          <input
            type="text"
            className={errors.firstName?"text-input error-border-color":"text-input generic-border-color"}
            onChange={(e) => {
              setFirstName(e.target.value);
            }}
            // onBlur={()=>validateSignUp("firstName")}
          />
          {errors.firstName && <span className="error-text font-size-6">{errors.firstName}</span>}
        </div>
        ...
        <div class="flex-column margin-bottom">
          ...
          <input
            type="text"
            className={errors.lastName?"text-input error-border-color":"text-input generic-border-color"}
            // required
            onChange={(e) => {
          ...
          />
          {errors.lastName && <span className="error-text font-size-6">{errors.lastName}</span>}
        </div>
        <div class="flex-column margin-bottom">
          ...
          />
           {errors.email && <span className="error-text font-size-6">{errors.email}</span>}
        </div>
        <div class="flex-column margin-bottom">
          <label class="font-size-6 font-bold-1">Password</label>
          <input
            type="password"
            className= {errors.password?"text-input error-border-color":"text-input generic-border-color"}
            required
            onChange={(e) => {
              setPassword(e.target.value);
            }}
          />
          {errors.password && <span className="error-text font-size-6">{errors.password}</span>}
        </div>
        <div>
          <button
